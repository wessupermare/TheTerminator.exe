...
# Date: 05/28/2015
#
# -Updated: 5/29/2015: Added in better functionality to check if a service actually stopped, and if not (after 20 seconds) find and kill PIDs.
...
$sb = { 
    ...
    $session = New-PSSession -ComputerName $svr
...
foreach($server in $serverNames) {
    $PSinstance = [powershell]::Create().AddScript($sb).AddArgument($server).AddArgument($startStop).AddArgument($services).AddArgument($processes).AddArgument($startup)
